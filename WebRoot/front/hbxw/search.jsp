...
<body>
	<div class="allbody">
            ...
            <div style=" margin-top:10px;">
            	...
            	<div class="fr" style="width:744px;">
                	<div class="lxwm" style=" width:744px; height:40px;">
                        ...
                        <div class="fr">您现在的位置：<a href="<%=basePath%>index/homAct!home.action">首页</a> > <a href="<%=basePath%>news/epNewAct!list.action">环保新闻</a><%if(!((abc==null)||abc.trim().equals(""))){%> > <%}%><%=abc%></div>
                		<div class="clear"></div>
            		</div>
                    <div>
                    	...
                    	<select name="column">
                    		...
                    		<option value="会议动态" <%if(abc.equals("会议动态")){ %>selected="selected" <% }%>>会议动态</option>
                    		<option value="宣传活动" <%if(abc.equals("宣传活动")){ %>selected="selected" <% }%>>宣传活动</option>
                    		<option value="问答互动" <%if(abc.equals("问答互动")){ %>selected="selected" <% }%>>问答互动</option>
                    		<option value="开放日活动"<%if(abc.equals("开放日活动")){ %>selected="selected" <% }%>>开放日活动</option>
                    	</select>
     ...
     <div class="page_info" style="margin-top:10px;">
		...
		<span>　　　　　　　　　　　　　　　　　　　</span>
     <a href="<s:url value="/news/epNewAct!fsearch.action"><s:param name="key" value="%{key}"/><s:param name="column" value="%{column}"/><s:param name="method" value="%{'first'}"/><s:param name="pager.currentPage" value="pager.currentPage"/></s:url>"><span style="background:#DCD9D4;">首页</span></a>
     <a href="<s:url value="/news/epNewAct!fsearch.action"><s:param name="key" value="%{key}"/><s:param name="column" value="%{column}"/><s:param name="method" value="%{'back'}"/><s:param name="pager.currentPage" value="pager.currentPage"/></s:url>"><span style="background:#DCD9D4;">上一页</span></a>
     <span style="background:#EBEBEA;">&nbsp;<s:property value="pager.currentPage" />&nbsp;</span>
     <a href="<s:url value="/news/epNewAct!fsearch.action"><s:param name="key" value="%{key}"/><s:param name="column" value="%{column}"/><s:param name="method" value="%{'next'}"/><s:param name="pager.currentPage" value="pager.currentPage"/></s:url>"><span style="background:#DCD9D4;">下一页</span></a>
     <form action="<%=basePath%>news/epNewAct!fsearch.action?key=${key}&column=${column}" method="post" id="gotoform" style="margin-bottom: 0;float:right;">
...
<script type="text/javascript">
	function toPage() {
	 ...
	 <a href="<s:url value="/news/epNewAct!fsearch.action"><s:param name="key" value="%{key}"/><s:param name="column" value="%{column}"/><s:param name="method" value="%{'last'}"/><s:param name="pager.currentPage" value="pager.currentPage"/></s:url>"><span style="background:#DCD9D4;">尾页</span></a>
       </div>
                    </div>
                </div>
