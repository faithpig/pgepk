...
<body>
	<div class="allbody">
        ...
        <div class="menu">
            <div style=" margin-top:10px;">
            	<div class="fl" style=" padding-left:0px; width:249px;">
                    ...
                    <div class="zs_zj">
                        <div>
                            <div class="fl xw">环保政策法规分类</div>
                            <div class="fr"></div>
                            <div class="clear"></div>
                        </div>
                        <div class="hx"></div>
                        <ul class="nynews3">
                            <li><a href="<%=basePath%>policy/epPolAct!fsearch.action?column=法律法规&pager.currentPage=0">法律法规</a></li>
                            <li><a href="<%=basePath%>policy/epPolAct!fsearch.action?column=导则标准&pager.currentPage=0">导则标准</a></li>
                            <li><a href="<%=basePath%>policy/epPolAct!fsearch.action?column=政策解读&pager.currentPage=0">政策解读</a></li>
                        </ul>
                    </div>
            	...
            	<div class="fr" style="width:744px;">
                	<div class="lxwm2" style=" width:744px; height:40px;">
                        <div class="fl lxwm_z">环保政策法规</div>
                        <%String abc=(String)ActionContext.getContext().get("column"); %>
                        <div class="fr">您现在的位置：<a href="<%=basePath%>index/homAct!home.action">首页</a> > <a href="<%=basePath%>policy/epPolAct!list.action">环保政策法规</a><%if(!((abc==null)||abc.trim().equals(""))){%> > <%}%><%=abc%></div>
                		<div class="clear"></div>
            		</div>
                    <div>
                    	<div align="right" style="padding-top:16px;">
                    	<form action="<%=basePath%>policy/epPolAct!fsearch.action" method="get">
                    	请输入检索内容：<input name="key" type="text" />
                    	<select name="column">
                    		<option value=" ">所有</option>
                    		<option value="法律法规" <%if(abc.equals("法律法规")){ %>selected="selected" <% }%>>法律法规</option>
                    		<option value="导则标准" <%if(abc.equals("导则标准")){ %>selected="selected" <% }%>>导则标准</option>
                    		<option value="政策解读" <%if(abc.equals("政策解读")){ %>selected="selected" <% }%>>政策解读</option>
                    	</select>
                    	...
                    	<ul class="newslist">
                        	<li class="newstop"><a href="javascript:void(0)" class="fl">环保政策法规记录</a><span class="fl">浏览量</span><span class="fr">创建时间</span></li>
                        	<s:iterator value="list" id="entity">
                        	<li><a href="<%=basePath%>policy/epPolAct!detail.action?id=<s:property value="id"/>" class="fl hide"><s:property value="title"/></a><span class="fl"><s:property value="count"/></span><span class="fr">[${entity.addTime}]</span></li>
                        	</s:iterator>
                        </ul>
     ...
     <a href="<s:url value="/policy/epPolAct!fsearch.action"><s:param name="key" value="%{key}"/><s:param name="column" value="%{column}"/><s:param name="method" value="%{'first'}"/><s:param name="pager.currentPage" value="pager.currentPage"/></s:url>"><span style="background:#DCD9D4;">首页</span></a>
     <a href="<s:url value="/policy/epPolAct!fsearch.action"><s:param name="key" value="%{key}"/><s:param name="column" value="%{column}"/><s:param name="method" value="%{'back'}"/><s:param name="pager.currentPage" value="pager.currentPage"/></s:url>"><span style="background:#DCD9D4;">上一页</span></a>
     <span style="background:#EBEBEA;">&nbsp;<s:property value="pager.currentPage" />&nbsp;</span>
     <a href="<s:url value="/policy/epPolAct!fsearch.action"><s:param name="key" value="%{key}"/><s:param name="column" value="%{column}"/><s:param name="method" value="%{'next'}"/><s:param name="pager.currentPage" value="pager.currentPage"/></s:url>"><span style="background:#DCD9D4;">下一页</span></a>
     <form action="<%=basePath%>policy/epPolAct!fsearch.action?key=${key}&column=${column}" method="post" id="gotoform" style="margin-bottom: 0;float:right;">
...
<script type="text/javascript">
	function toPage() {
	 ...
	 <a href="<s:url value="/policy/epPolAct!fsearch.action"><s:param name="key" value="%{key}"/><s:param name="column" value="%{column}"/><s:param name="method" value="%{'last'}"/><s:param name="pager.currentPage" value="pager.currentPage"/></s:url>"><span style="background:#DCD9D4;">尾页</span></a>
       </div>
                    </div>
                </div>
